...
-- It worked - maybe.
      ELSE
        CALL fgl_winMessage(
            "Error", "Failed to load '" || l_4rp || "'", "exclamation")
        CONTINUE WHILE
      END IF
      MESSAGE "Processing..."
      ...
      IF NOT fgl_report_runReportFromProcessLevelDataFile( l_myHandler, l_xmlFile)
          THEN
        CALL fgl_winMessage(
            "Error",
...
--------------------------------------------------------------------------------
FUNCTION init_gre(
    l_inFile, l_4rp, l_device, l_preview, l_targetName, l_merge_cells,
    l_singleSheet)
  ...
  IF l_device = "PDF" THEN
    ...
    IF l_fontDirectory.getLength() > 1 THEN
      ...
      CALL fgl_report_configurePDFDevice(
          l_fontDirectory, l_antialiasFonts, l_antialiasShapes, l_monochrome,
          l_fromPage, l_toPage)
    ELSE
